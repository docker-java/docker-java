...
public class ListConfigCmdExecIT extends SwarmCmdIT {
    ...
    public void tesListConfig() throws DockerException {
        ...
        CreateConfigResponse exec = dockerRule.getClient().createConfigCmd(new ConfigSpec().withName(configName).withData("mon config en clair".getBytes())).exec();
        ...
        List<Config> configsAfterRemoved = dockerRule.getClient().listConfigsCmd()
                .withNameFilter(Lists.newArrayList(configName))
                .exec();
