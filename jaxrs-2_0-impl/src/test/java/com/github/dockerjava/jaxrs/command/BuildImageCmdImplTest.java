...
public class BuildImageCmdImplTest extends AbstractDockerClientTest {
    ...
    public void testDockerBuilderAddUrl() throws Exception {
        ...
        MatcherAssert.assertThat(response, Matchers.containsString("Docker"));
    }
    ...
    public void testNetCatDockerfileBuilder() throws Exception {
        ...
        Assert.assertNotNull(imageId, "Not successful in build");

        InspectImageResponse inspectImageResponse = dockerClient.inspectImageCmd(imageId).exec();
        MatcherAssert.assertThat(inspectImageResponse, Matchers.not(Matchers.nullValue()));
        MatcherAssert.assertThat(inspectImageResponse.getId(), Matchers.not(Matchers.nullValue()));
        AbstractDockerClientTest.LOG.info("Image Inspect: {}", inspectImageResponse.toString());
        ...
        MatcherAssert.assertThat(inspectContainerResponse.getId(), Matchers.notNullValue());
        MatcherAssert.assertThat(inspectContainerResponse.getNetworkSettings().getPorts(), Matchers.notNullValue());
