...
public class DockerConfigFile {
    ...
    public static DockerConfigFile loadConfig(File dockerConfigPath) throws IOException {
        ...
        DockerConfigFile dockerConfig = null;

        //parse new docker config file format
        if (dockerCfgFile.exists() && dockerCfgFile.isFile()) {
            try {
                dockerConfig = MAPPER.readValue(dockerCfgFile, DockerConfigFile.class);
            } catch (IOException e) {
        ...
        if (dockerConfig == null) {
            dockerCfgFile = new File(dockerConfigPath, File.separator + DOCKER_LEGACY_CFG);

            if (dockerCfgFile.exists() && dockerCfgFile.isFile()) {
                Map<String, AuthConfig> configMap = null;
                try {
                    dockerConfig = new DockerConfigFile(MAPPER.<Map<String, AuthConfig>>readValue(dockerCfgFile, CONFIG_MAP_TYPE));
                } catch (IOException e) {
                    // pass
                }
            }
        ...
        if (dockerConfig == null) {
            ...
            if (dockerCfgFile.exists() && dockerCfgFile.isFile()) {
                List<String> authFileContent = FileUtils.readLines(dockerCfgFile, StandardCharsets.UTF_8);
                ...
                dockerConfig = new DockerConfigFile(new HashMap<>(Collections.singletonMap(config.getRegistryAddress(), config)));
            }
        }
