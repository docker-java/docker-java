...
public class ListServicesCmdExecIT extends SwarmCmdIT {
    ...
    public void testListServices() throws DockerException {
        ...
        List<Service> services = docker1.listServicesCmd().withIdFilter(serviceId).exec();
        assertThat(services, hasSize(1));
        //filtering with service name
        services = docker1.listServicesCmd().withNameFilter(SERVICE_NAME).exec();
        assertThat(services, hasSize(1));
        //filter labels
        services = docker1.listServicesCmd().withLabelFilter(serviceLabels).exec();
        assertThat(services, hasSize(1));
        services = docker1.listServicesCmd().withLabelFilter(LABEL_KEY + "=" + LABEL_VALUE).exec();
        assertThat(services, hasSize(1));
